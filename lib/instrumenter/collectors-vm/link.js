...
function getInstanceNicsInfo(instanceUuid, cb) {
    ...
    setTimeout(function _to() {
        done(new Error('Timed out waiting for vminfod'));
    }, VMINFOD_TIMEOUT_MS);

    var url = 'http://127.0.0.1:9090';
    var client  = mod_restify.createJsonClient(url);
    client.get('/vms/' + instanceUuid,
        function _processHttpResp(err, _req, _res, obj) {
        if (err) {
            done(new Error('Failed to retrieve instance ' +
                'info from vminfod: ' + err));
            return;
        }
        var nics = Array.isArray(obj.nics) ? obj.nics : [];
        done(null, nics);
    });
}
...
LinkMetricCollector.prototype.getMetrics = function getMetrics(opts, callback) {
    ...
    getInstanceNicsInfo(opts.zInfo.zonename,
        function _processInstanceNicsInfo(err, nics) {
        if (err) {
        ...
        kstat_common.kstatsToMetrics({
            ...
            kstatLabeler: function _labelLink(linkKstat) {
                ...
                //
                // because of: https://github.com/TritonDataCenter/illumos-joyent/blob/release-20170928/usr/src/uts/common/io/dls/dls_mgmt.c#L784
                //
                // And because of:
                //
                //   https://github.com/TritonDataCenter/illumos-joyent/blob/release-20170928/usr/src/cmd/dlmgmtd/dlmgmt_util.c#L701
                //
