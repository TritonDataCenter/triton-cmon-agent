...
function getInstanceNicsInfo(instanceUuid, cb) {
    ...
    timeout.id = setTimeout(function _to() {
    ...
    mod_http.get('http://127.0.0.1:9090/vms/' + instanceUuid,
        ...
        if (res.statusCode !== 200) {
            res.resume();    // Consume res stream
            done(new Error('Failed to retrieve instance info from vminfod ' +
        ...
        res.on('end', function _onEnd() {
            try {
                ...
                done(new Error('Invalid payload received from vminfod: ' +
                    vmInfo));
            }
        });
