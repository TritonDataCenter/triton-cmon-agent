 ...
 */
function zpoolHealthEnc(health) {
    var zpoolStatus = {
        'ONLINE': '0', 'DEGRADED': '1', 'FAULTED': '2',
        'OFFLINE': '3', 'REMOVED': '4', 'UNAVAIL': '5'
    };

    return zpoolStatus[health] || '-1';
}
    ...
    forkExecWait({
        ...
        for (var i = 0; i < lines.length; i++) {
            ...
            metrics = keys.map(function _createMetrics(_, ind) {
                ...
                if (keys[ind].encoder) {
                    metric.value = keys[ind].encoder(metric.value);
                }
