...
function translateTime(timeVal) {
    ...
    // ntpq can return the string `(no time)' in some rare cases where an
    // event has not yet occurred for the running process. Most people never
    // encounter this, but those that do will get it chronically. Because this
    // is normal behavior for ntpq, we need to be able to express this in a
    // format that's usable by exposition, which means we have to emit a
    // numberic value because this metric is a gauge. Any numeric value means a
    // time, even negative ones. So we'll return 0, the epoch, January 1, 1970.
    // Metrics collection agents SHOULD discard this value as too low, which
    // would cause the graph to display this as a gap. Even if they don't, the
    // graph will drop to 0, which I think also gets the point across fine.
    //
    // Callers of translateTime are supposed to split the time string so that
    // only the hex portion is passed in, but we'll make sure this is handled
    // properly even if that doesn't happen.
    var bogusVals = [
        '(no', '(no time)'
    ];
    if (bogusVals.indexOf(timeVal) !== -1) {
        return (0);
    }
...
function addStatLine(state, ntpData, line) {
    ...
    /* BEGIN JSSTYLED */
    matches = line.match(/([a-z\_]+=[A-Za-z0-9\.()_+-]+)/g);
    /* END JSSTYLED */
    if (matches) {
        ...
        mod_assert.equal(line, '', 'unexpected output line from ntpq: ' + line);
    }
}
