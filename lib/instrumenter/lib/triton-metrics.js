    ...
    getTritonMetadata(opts, function gotMetadata(err, metadata) {
        ...
        if (metadata.adminIp && metadata.metricPorts) {
            ...
            mod_vasync.forEachParallel({
                ...
                /*
                 * If at least one metricPort scrape succeeded, we treat
                 * the entire request as a success rather than a failure and
                 * include a warning in the log -- if we were to return an error
                 * here, the master collector would discard the metrics that we
                 * did get successfully.
                 *
                 * This is a short-term workaround - see TRITON-1285 for a
                 * discussion of long-term solutions.
                 */
                if (_err) {
                    var numSuccesses = results.successes.length;
                    if (results.successes.length === 0) {
                        callback(_err);
                        return;
                    }

                    var partialSuccessMsg =
                        'some Triton metrics requests failed. ' +
                        'Treating as success.';
                    opts.log.warn({
                        err: _err,
                        failures: results.operations.length - numSuccesses,
