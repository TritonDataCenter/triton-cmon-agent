...
/*
 * Takes a number of kibibytes and returns a number of bytes as a string.
 */
function kibibytesToBytes(value) {
    mod_assert.number(value, 'value');

    // Use mod_big because we want to avoid floating-point weirdness.
    return (mod_big(value).div(1024).toString());
}
...
/*
 * Takes a number of usec and returns a number of seconds as a string.
 */
function usecToSec(value) {
    mod_assert.number(value, 'value');

    // Use mod_big because we want to avoid floating-point weirdness.
    return (mod_big(value).div(1000000).toString());
}

/*
 * Takes a number of msec and returns a number of seconds as a string.
 */
function msecToSec(value) {
    mod_assert.number(value, 'value');

    // Use mod_big because we want to avoid floating-point weirdness.
    return (mod_big(value).div(1000).toString());
}

module.exports = {
    calculateLoadAvg: calculateLoadAvg,
    fetchRunningZones: fetchRunningZones,
    kibibytesToBytes: kibibytesToBytes,
    memLimit: memLimit,
    msecToSec: msecToSec,
    usecToSec: usecToSec
};
